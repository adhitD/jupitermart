...
<body>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      const extraOffset = 15; // jarak tambahan agar section sedikit ke bawah

      const urlParams = new URLSearchParams(window.location.search);
      const keyword = urlParams.get('q');

      if (keyword) {
        const storeSection = document.getElementById('official-store');
        if (storeSection) {
          const scrollPos = storeSection.getBoundingClientRect().top + window.pageYOffset - navbarHeight + extraOffset;
          window.scrollTo({
            top: scrollPos,
            behavior: 'smooth'
          });
